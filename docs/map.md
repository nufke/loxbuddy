...
As the Webpage element has no inputs, the specified map is *fixed* and cannot be altered afterwards.
...
In order to visualize multiple markers on the map, the following JSON object structure could be used: 
...
The object should be send as *stringified* text message to the Virtual Text Marker. Note that we again replace `#` by a `%23` to avoid interpretation issues.
