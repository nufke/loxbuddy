...
<div class="w-full flex align-center justify-center">
	<svg width="320" height="320" on:mouseup={mouseUp} on:mousemove={mouseMove}>
		<circle class="track" cx={loc.x} cy={loc.y} r={radius} on:mousedown={trackMouseDown}/>
		<circle class="set" cx={set.x} cy={set.y} r="12"/>
		<circle class="knob" cx={knob.x} cy={knob.y} r="12" on:mousedown={knobMouseDown}/>
		...
		<svg class="button" x="105" y="260" on:mousedown={startDown} on:mouseup={endDown}>
			<circle cx="20" cy="20" r="20" fill="black"/>
			<path stroke-width="2" stroke-linecap="round" stroke-linejoin="round" fill="none" stroke="currentColor" d="m14 17 6 6 6-6"></path>
		</svg>
		<svg class="button" x="175" y="260" on:mousedown={startUp} on:mouseup={endUp}>
			<circle cx="20" cy="20" r="20" fill="black"/>
