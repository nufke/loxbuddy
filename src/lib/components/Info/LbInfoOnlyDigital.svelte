<script lang="ts">
	...
	let statusName = $derived(control.details?.text[controlState]);
	let statusColor = $derived(control.details?.color[controlState]);
	...
	<LbDialog open={controlOpen} onClose={closeControl} {control} title={control.name}>
		{#snippet description()}
			<div class="flex flex-col items-center justify-center">
				...
				<p class="text-lg" style="color: {statusColor}">{statusName}</p>
			</div>
		{/snippet}
	</LbDialog>
{/if}
