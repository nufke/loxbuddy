<script lang="ts">
	...
	let mapInfo = $derived(textAndIcon.match(/openstreetmap.org\/#map=(.*)\/(.*)\/(.*)/) || []);
	...
	let controlView: ControlView = $derived({
		...
		iconName: mapInfo.length ? 'loxbuddy:location' : controlStore.getIcon(control, controlOptions.isSubControl, iconAndColor),
		iconColor: iconAndColor?.color || 'dark:fill-surface-50 fill-surface-950',
		textName: control.name,
		statusName: mapInfo.length ? `${mapInfo[2]}, ${mapInfo[3]}` : textAndIcon,
		links: control.links,
		dialog: dialog,
	});
