<script lang="ts">
	...
	*/
	...
	const type = Number(control.details?.animation);
	const closeIcon = buttonMapLeft[type];
	const openIcon = buttonMapRight[type];
	...
	let iconColor = $derived(position > 1 ? 'dark:text-primary-500 text-primary-700' : 'text-surface-950 dark:text-surface-50');
	let statusName = $derived(
		position > 99 ? $_('Opened') :
		(position < 1 ? $_('Closed') : fmt.sprintf('%2.0f%% %s', position, $_('Opened')))
	);
	let statusColor = $derived(position > 1 ? 'dark:text-primary-500 text-primary-700' : 'dark:text-surface-300 text-surface-700');
	...
	{#snippet actions()}
		<div class="flex flex-row items-center gap-2 mr-1">
			<button type="button" class="btn-icon w-[18px] h-[18px] p-3 bg-surface-50-950 rounded-lg border border-white/15 hover:border-white/50 active:bg-primary-500"
					onclick={(e) => { e.stopPropagation(); e.preventDefault(); handleSecuredClick((pw) => controlStore.setControl(control, 'close', pw)); }}>
				<LbIcon name={closeIcon}/>
			</button>
			<button type="button" class="btn-icon w-[18px] h-[18px] p-3 bg-surface-50-950 rounded-lg border border-white/15 hover:border-white/50 active:bg-primary-500"
					onclick={(e) => { e.stopPropagation(); e.preventDefault(); handleSecuredClick((pw) => controlStore.setControl(control, 'open', pw)); }}>
				<LbIcon name={openIcon}/>
			</button>
		</div>
	{/snippet}
</LbControl>
			...
			<div class="flex flex-col items-center justify-center">
				...
				<div class="w-full mt-3 grid grid-cols-2 gap-2">
					<button type="button"
							...
							onclick={(e) => { e.stopPropagation(); e.preventDefault(); handleSecuredClick((pw) => controlStore.setControl(control, 'close', pw)); }}>
						<span class="text-lg">{$_('Close')}</span>
					</button>
					<button type="button"
							class="w-full btn btn-lg h-[48px] bg-surface-50-950 shadow-sm rounded-lg border border-white/15 hover:border-white/50 active:bg-primary-500"
							onclick={(e) => { e.stopPropagation(); e.preventDefault(); handleSecuredClick((pw) => controlStore.setControl(control, 'open', pw)); }}>
						<span class="text-lg">{$_('Open')}</span>
					</button>
					<button type="button"
							...
							onclick={(e) => { e.stopPropagation(); e.preventDefault(); handleSecuredClick((pw) => controlStore.setControl(control, 'partiallyOpen', pw)); }}>
						<span class="text-lg">{$_('Open partially')}</span>
					</button>
				</div>
			</div>
		{/snippet}
